# Hi, I'm Pratik 👋

**Cloud Architect | Full-Stack Developer | Problem Solver**

Final-year Computer Science student at MIT ADT University, Pune with a passion for building scalable cloud solutions and enterprise applications. Dedicated to mastering Data Structures & Algorithms with a **498-day LeetCode streak** and deep expertise in cloud technologies.

---

## 🎯 About Me

I'm a committed software engineer with a strong foundation in **Data Structures & Algorithms** and proven dedication to continuous learning. My **500-days LeetCode streak** demonstrates my commitment to coding excellence and problem-solving. I specialize in:

- **Cloud Architecture** and AWS services
- **Backend Development** with Java and enterprise frameworks
- **Full-Stack Development** with modern technologies
- **System Design** and scalable solutions

Currently focused on becoming a proficient Cloud Architect while maintaining excellence in competitive programming and building production-grade applications.

---

## 💼 Technical Skills & Expertise

**Cloud & DevOps**
- AWS Services & Cloud Architecture
- Microservices & Container Orchestration
- Infrastructure as Code (IaC)
- System Design & Scalability

**Backend Development**
- Java (Advanced) | Spring Boot | Spring Security
- RESTful APIs | Microservices Architecture
- Database Design & Optimization
- Performance Tuning

**Data Structures & Algorithms**
- Advanced DSA (498-day LeetCode streak)
- Algorithm Optimization
- Competitive Programming
- Problem-Solving & Complexity Analysis

**Frontend Development**
- HTML5 | CSS3 | JavaScript
- React | Responsive Design
- UI/UX Fundamentals

**Development Tools & Practices**
- Git & Version Control
- CI/CD Pipelines
- Agile Development
- Code Quality & Best Practices

---

## 🏆 Achievements & Highlights

- **498-Day LeetCode Streak** - Consistent problem-solving and algorithm mastery
- **Advanced Java Proficiency** - Object-oriented design, concurrency, and enterprise patterns
- **Cloud Architecture Knowledge** - AWS services, microservices design, and scalability patterns
- **Full-Stack Capabilities** - End-to-end application development from backend to frontend

---

## 🚀 What I'm Looking For

- **Internships/Entry-level roles** in Cloud Engineering, Backend Development, or Full-Stack Engineering
- Opportunities to contribute to **enterprise-scale projects** with emphasis on performance and scalability
- Roles that value **continuous learning**, **code quality**, and **problem-solving**
- Positions in **Fintech, SaaS, Cloud-Native companies, or Product-Based organizations**
- Teams where I can leverage my DSA expertise and cloud knowledge

---

## 📫 Connect With Me

- **Email**: [pratikraj2000qwerty@gmail.com](mailto:pratikraj2000qwerty@gmail.com)
- **LinkedIn**: [linkedin.com/in/prpratikraj](https://www.linkedin.com/in/prpratikraj/)
- **LeetCode**: [leetcode.com/u/pratikraj2000qwerty](https://leetcode.com/u/pratikraj2000qwerty/)
- **WebPage**:[pratikraj.dev](https://pratikraj.dev)

---

## 💡 Open to Collaboration

I'm always interested in discussing:
- Software architecture and system design
- Cloud solutions and AWS best practices
- Advanced algorithms and competitive programming
- Full-stack application development
- Open-source contributions

Feel free to reach out—let's collaborate on building something great!

**Let's connect and create impact together! 🚀**
